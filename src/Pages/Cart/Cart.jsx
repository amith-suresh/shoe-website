...
const Cart = () => {
  const { cartItems, PRODUCTS, removeFromCart, getTotalCartAmount,calculateTotalPrice } = useContext(StoreContext);
  ...
  return (
    <div className='cart'>
      <div className='cart-items p-4 md:p-6 lg:p-8'>
        <div className="cart-items-title grid grid-cols-6 gap-4">
          ...
          if (cartItems[item.id]) {
            return (
              <div key={item.id} className='grid grid-cols-6 gap-4 items-center'>
                ...
                <p className='col-span-1'>{cartItems[item.id]}</p>
              </div>
            );
          }
          return null;
